...
OperationsEnum GetOperation(std::string ops)
{
    static const std::map<std::string, OperationsEnum> operationsStrings{
        ...
        {"regcallback", OperationsEnum::RegisterCallback},
        {"unregcallback", OperationsEnum::UnregisterCallback},
        {"subdataref", OperationsEnum::SubscribeDataref},
...
void Callback(double step, void* tag)
{
    ...
    while(cm->GetMessageQueueLenght() > 0)
    {
        ...
        case OperationsEnum::RegisterCallback:
        {
            ///Needed info : a time interval
            cm->GetLogger().Log("Registering callback");
            if (!m.message.contains("Interval") || !m.message["Interval"].is_number())
            {
